...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Set the u_height value of deviceinfo to 'size'.
        The 'size' will be out in next migration"""
        for deviceinfo in orm.DeviceInfo.objects.filter(
                 ~models.Q(size=0), size__isnull=False).all():
            deviceinfo.u_height = deviceinfo.size
            deviceinfo.save()

    def backwards(self, orm):
        "Do nothing. The data can stay merged if we go back"

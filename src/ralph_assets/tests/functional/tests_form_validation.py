...
class TestValidations(TestCase):
    ...
    def _get_post_data(self, id, *args, **kwargs):
        return {
            'form-{0}-id'.format(id): id,
            'form-{0}-type'.format(id):\
                kwargs.get('type', AssetType.data_center.id),
            'form-{0}-model'.format(id):\
                kwargs.get('model', self.model1.id),
            'form-{0}-invoice_no'.format(id):\
                kwargs.get('invoice_no', 'Invoice No0'),
            'form-{0}-invoice_date'.format(id):\
                kwargs.get('invoice_date', '2014-01-01'),
            'form-{0}-order_no'.format(id):\
                kwargs.get('order_no', 'Order No0'),
            'form-{0}-sn'.format(id):\
                kwargs.get('sn', '0000-0000-0000-0000'),
            'form-{0}-barcode'.format(id):\
                kwargs.get('barcode', 'bc-0000'),
            'form-{0}-support_period'.format(id):\
                kwargs.get('support_period', 24),
            'form-{0}-support_type'.format(id):\
                kwargs.get('support_type', 'standard0'),
            'form-{0}-support_void_reporting'.format(id):\
                kwargs.get('support_void_reporting', 'on'),
            'form-{0}-provider'.format(id):\
                kwargs.get('provider', 'Provider 0'),
            'form-{0}-status'.format(id):\
                kwargs.get('status', AssetStatus.in_progress.id),
            'form-{0}-source'.format(id):\
                kwargs.get('source', AssetSource.shipment.id),
            'form-{0}-ralph_device_id'.format(id):\
    ...
    def test_send_wrong_data_in_bulkedit_form(self):
        ...
        post_data.update(
            2,
            {
                ...
                'order_no': 'Order No2',
                'form-1-invoice_date': '2014-01-01',
            }
        )
            'form-1-id': 2,
            'form-1-model': '',
    ...
    def test_send_wrong_data_in_bulkedit_form(self):
        ...
        bulk_data = [
            ...
            dict(
                row=1, field='invoice_date', error='Invoice date cannot be empty.',
            ),
            dict(
